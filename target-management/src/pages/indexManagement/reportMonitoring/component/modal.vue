  ...
  <div>
    ...
    >
      ...
      >
        ...
        <!-- <a-form-model-item label="运行状态" ref="status" prop="status">
          <a-select
            v-model="form.status"
            placeholder="请选择运行状态"
            allow-clear
          ...
          >
            <a-select-option v-for="item in nationOption" :key="item.name">
              {{ item.value }}
            </a-select-option>
          </a-select>
        </a-form-model-item> -->
...
<script>
import { getMonitoringDataUpdLists, getAllLists } from "@/api/management";
import globalModal from "@/components/globalModal/globalModal";
export default {
  components: {
    globalModal
  },
  props: ["form", "options", "treeData"],
  data() {
    return {
      treeExpandedKeys: [],
      value: undefined,
  ...
  methods: {
    ...
    async meatData() {
      this.nationOption.forEach(item => {
        if (this.form.status == item.value) {
          this.form.status = item.name;
        }
      });
      // console.log(this.form);
      this.form.upPerson = sessionStorage.getItem("uCenterName");
      let params = this.form;
      let res = await getMonitoringDataUpdLists(params);
      if (res.code == 200) {
