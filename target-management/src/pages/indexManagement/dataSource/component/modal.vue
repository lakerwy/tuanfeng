    ...
    >
      ...
      >
        ...
        <!-- <a-form-model-item label="集群名称" prop="clustername">
          <a-input placeholder="请输入集群名称" v-model="form.clustername" />
        </a-form-model-item> -->
                <a-form-model-item> 
...
import { getdataSourceUpdLists,getdataSourceTestLists } from "@/api/management";
export default {
  components: {
    globalModal
  },
  ...
  methods: {
    async meatData() {
      ...
      let res = await getdataSourceUpdLists(params);
      if (res.code == 200) {
