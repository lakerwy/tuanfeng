    ...
    >
      ...
      >
        <a-form-model-item label="预警等级" ref="warnLevel" prop="warnLevel">
          <a-input placeholder="请输入预警等级" v-model="form.warnLevel" />
        </a-form-model-item>
        <a-form-model-item label="最小值" ref="valueMin" prop="valueMin">
          <a-input placeholder="请输入最小值" v-model="form.valueMin" />
        </a-form-model-item>
        <a-form-model-item label="最大值" ref="valueMax" prop="valueMax">
          <a-input placeholder="请输入最大值" v-model="form.valueMax" />
        </a-form-model-item>
        <a-form-model-item label="颜色标识" ref="color" prop="color">
          <colorPicker
            v-model="form.color"
            @change="headleChangeColor(form.color)"
          />
  ...
  data() {
    ...
    let minValue = (rule, value, callback) => {
      ...
      if (this.form.valueMax) {
        let maxValue = parseInt(this.form.valueMax);
        if (value > maxValue) {
    ...
    let maxValue = (rule, value, callback) => {
      ...
      if (this.form.valueMin) {
        let minValue = parseInt(this.form.valueMin);
        if (value < minValue) {
    ...
    return {
      ...
      rules: {
        ...
        valueMin: [
          ...
          { validator: minValue, trigger: "blur" }
        ],
        valueMax: [
