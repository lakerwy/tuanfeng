<template>
	<a-modal v-if="footer" :class="[modalClass, simpleClass]" :visible="showModal" :title="title" @ok="handleOk" @cancel="handleCancel"
	 :width="width == '100%' ? '70%' : width">
		<slot name="modalBody" style="minheight: 600px"></slot>
	</a-modal>
	<a-modal v-else :class="modalClass" :visible="showModal" :title="title" @ok="handleOk" @cancel="handleCancel" :width="width == '100%' ? '40%' : width"
	 :footer="null">
  ...
  var footer =null
	export default {
		...
		methods: {
            ...
            footer = modalContent.getElementsByClassName('ant-modal-footer')[0]
