        ...
        <div class="tree">
          ...
          >
            <span class="custom-tree-node" slot-scope="{ node }">
              <img
                v-if="node.data.filetype === 'PDF'"
                :src="imgSrcPdf"
                alt=""
              />
              <img
                v-if="node.data.filetype === 'TXT'"
                :src="imgSrcTxt"
                alt=""
              />
              <img
                v-if="node.data.filetype === 'JPG'"
                :src="imgSrcJpg"
                alt=""
              />
              <img
                v-if="node.data.filetype === 'MDB'"
                :src="imgSrcMdb"
                alt=""
              />
              <img
                v-if="node.data.filetype === 'GDB'"
                :src="imgSrcGdb"
                alt=""
              />
              <img
                v-if="node.data.filetype === 'DWG'"
                :src="imgSrcDwg"
                alt=""
              />
              <img
...
import imageMdb from "../qualityDetail/components/imageMdbTree";
import imageGdb from "../qualityDetail/components/imageGdb";
import tabsView from "../../components/tabsView/index";
import { mapGetters, mapMutations } from "vuex";
import { treeDataList } from "../../api/auditTaskOthers";
// import { getTableTitalLists } from "../../api/mdb";
...
export default {
  ...
  data() {
    return {
      ...
      // pdfSrc: "",
      pdfSrc: "",
      imgSrcPdf: require("../../assets/imgs/PDF.png"),
      imgSrcTxt: require("../../assets/imgs/txt.png"),
      imgSrcJpg: require("../../assets/imgs/jpg.png"),
      imgSrcMdb: require("../../assets/imgs/mdb.png"),
      imgSrcGdb: require("../../assets/imgs/gdb.png"),
      imgSrcXls: require("../../assets/imgs/xls.png"),
      imgSrcDwg: require("../../assets/imgs/dwg.png"),
      treeKeyword: "",
      ...
      versionOptions: [],
      form: {},
      name: JSON.parse(sessionStorage.getItem("currentTask")).taskName,
      mdbUrl: "",
      mdbId:""
    };
  },
  ...
  methods: {
    ...
    handleTreeData(data) {
      ...
      data.forEach(item => {
        if (item.parentId == 0) {
          item.metadataList = item.metadataList.concat(item.children);
        }
      });
      data.forEach(item => {
        if (item.children != null) {
          ...
          item.children.forEach(items => {
            if (!items.directoryName) {
              items.directoryName = items.filename;
            } else {
              items.children = items.metadataList;
            }
          });
        }
      });
      // console.log(result)
      if (result[0].children[0].fileType != "file") {
        this.pdfSrc = result[0].children[0].fileUrl;
      } else {
        this.pdfSrc = result[0].children[0].children[0].fileUrl;
      }
    ...
    handleNodeClick(obj, node, self) {
      ...
      if (obj.filetype !== "MDB" && obj.filetype !== "GDB") {
        if (node.data.fileType != "file") {
          this.isShow = true;
          this.pdfSrc = obj.fileUrl;
        }
      } else if (obj.filetype == "MDB") {
        this.isShow = false;
        this.isMdb = true;
        this.jsonUrl = obj.fileUrl;
        this.mdbId=obj.id
      } else if (obj.filetype == "GDB") {
        this.isShow = false;
        this.isMdb = false;
        this.mdbUrl = obj.fileUrl;
      }
    },
