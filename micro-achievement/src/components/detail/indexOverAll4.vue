<template>
  ...
  <div class="quality-detail-left">
    <div class="title">
      <el-radio v-model="radio" label="1">空间布局符合性审查</el-radio>
    </div>
    <div class="content">
      <el-form
        label-width="120px"
...
export default {
  ...
  mounted() {
    if (
      this.$parent.approveReqList[3].status == 0 ||
      this.$parent.approveReqList[3].status == 1
    ) {
      this.form.fileList = this.$parent.fileList4;
      this.form.status = this.$parent.approveReqList[3].status;
      this.form.remarks = this.$parent.approveReqList[3].remarks;
      this.form.attachmentDescription = this.$parent.approveReqList[3].attachmentDescription;
    }
        if(this.type=='12'){
      this.getDataLIst();
    }
  ...
  methods: {
    ...
    uploadChange(file, fileList) {
      ...
      this.$parent.fileList4 = fileList;
    ...
    async getUploadList() {
      ...
      this.$parent.approveReqList[3].attachmentId = this.form.attachmentId;
    ...
    handleClickPass() {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          this.$parent.approveReqList[3].status = 1;
          this.$parent.approveReqList[3].remarks = this.form.remarks;
          this.$parent.approveReqList[3].attachmentDescription = this.form.attachmentDescription;
          // console.log(this.$parent.approveReqList[3]);
          this.isBtn = false;
          this.isPass = true;
        } else {
