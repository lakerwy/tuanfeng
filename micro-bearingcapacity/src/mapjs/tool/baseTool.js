/**
import scaleDenominators from './../../../Dictionary/scale';
import scaleDenominators from './../../../Dictionary/scale';
 * 使用说明
 * 坐标定位 gotoCoordinate(map,[0,0]);
 * 范围或几何体定位 gotoGeomtryOrExtent(map,[0,0,100000,100000])
 * 放大 zoomIn(map);
 * 缩小 zoomOut(map);
 * 全屏 toggleFullScreen(map);
 ...
 * @param { ol.Map } map
 */
export function toggleFullScreen(map) {
  handleFullScreen_(map);
}

function handleFullScreen_(map) {
...
export function setResolutionByScale(map, scaleDenominator, center) {
  ...
  let r = scaleDenominator / metersPerUnit / (25.4 / 0.28); //  计算出分辨率
...
export function getScaleDenominatorByResolution(map) {
  ...
  let scaleDenominators = resolution * metersPerUnit * (25.4 / 0.28);
