...
import { getGovDataNumber } from "@/api/oneMap";
export default {
  data() {
    return {
      locale: zhCN,
  ...
  mounted() {
    registerProj();
    this.loadMap();
    // console.log(888,window.globalUrl)
  },
  ...
  methods: {
    ...
    async handleOk() {
      ...
      // formData.append('layerAliasname', this.currnetResourceId);
      // formData.append('year', this.searchYear);
      let params = {
        layerAliasname: this.currnetResourceId,
      ...
      if (code == 200) {
        this.govYearvisible = false;
        if (data.length == 0) this.$message.info("当前年份暂无政务数据！");
        debugger;
        this.addFeatureByArea(data);
      }
    },
    // 创建空的矢量图层
    createEmptyLayer() {
