...
import { queryArcGISSpatial } from "@/mapjs/Query/arcgisQuery.js"
 ...
 */
 export async function getCityByCoordinate(coordinate) {
  if (!coordinate) {
    return;
  }

  let cities_mapServer=window.globalUrl.districts.town
 ...
 * @param {*} item
 */
export function setTownLayer(info) {
  for (let index = 0; index < window.TF_XZ.features.length; index++) {
    if (window.TF_XZ.features[index].properties.ad_code == info.code) {
      let layer = new VectorLayer({
        source:new VectorSource({
          features:new GeoJSON().readFeatures(window.TF_XZ.features[index])
        })
      })
      if (layer) {
        layer.setStyle(townStyle);
        layer.set("layerName", "setTownLayer");
        return layer;
      }
      break;
    }
