  ...
  <div>
    <HeaderContent>
      <Form ref="searchForm"
        ...
        inline
        :label-width="110">
        <FormItem label="数据领域名称:" prop="name">
          <Input type="text" v-model="pageInfo.name" placeholder="请输入数据领域名称"/>
        </FormItem>
        <FormItem label="数据领域编码:" prop="code">
          <Input type="text" v-model="pageInfo.code" placeholder="请输入数据领域编码"/>
        </FormItem>
        <FormItem>
    ...
    <Card shadow>
      <div class="search-con search-con-top">
        ...
        <div>
          <Button  icon="md-add" type="primary" @click="handleModal('add')">添加数据领域</Button>
        </div>
      </div>
      <tree-table
        ref="tree"
        ...
        <template slot="operation" slot-scope="scope">
          <div class="actionIcon">
            <Tooltip content="添加下级数据领域" placement="top">
        ...
        <Tabs @on-click="handleTabClick" :value="current">
          <!-- <TabPane label="角色信息" name="form1"> -->
            <Form style="width:350px" v-show="current == 'form1'" ref="form1" :model="formItem" :rules="formItemRules" :label-width="110" label-position="right">
              <FormItem label="数据领域名称:" prop="dataName">
                <Input v-model.trim="formItem.dataName" placeholder="请输入数据领域名称" />
              </FormItem>
              <FormItem label="数据领域编码:" prop="dataCode">
                <Input v-model.trim="formItem.dataCode" placeholder="请输入数据领域编码" />
              </FormItem>
            </Form>
          <!-- </TabPane> -->
        </Tabs>
...
<script>
import { getDataDomain, setDataDomain, updateDataDomain, deleteDataDomain } from '@/api/catalogue'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      formItemRules: {
        dataName: [
          { required: true, message: '数据领域称不能为空', trigger: 'blur' },
          { min: 2, max: 20, message: '数据领域名称在2-20个字符之间', trigger: 'blur' },
          { pattern: /^[^\s]*$/, message: '数据领域名称不能输入空格', trigger: 'blur' },
          { validator: validateInput, trigger: 'blur' }
        ],
        dataCode: [
          { required: true, message: '数据领域编码不能为空', trigger: 'blur' },
          { min: 2, max: 20, message: '数据领域编码在2-20个字符之间', trigger: 'blur' },
          { pattern: /^[^\s]*$/, message: '数据领域编码不能输入空格', trigger: 'blur' },
          { validator: validateInput, trigger: 'blur' }
        ]
      },
      formItem: {
  ...
  methods: {
    ...
    async handleModal (status, data) {
      ...
      if (this.current === this.forms[0]) {
        if (status === 'edit') {
          this.modalTitle = '编辑数据领域 - ' + data.dataName
        } else {
          if (this.addChildren) {
            this.modalTitle = '添加下级数据';
            this.formItem = {};
        ...
        this.$refs[this.current].validate(async (valid) => {
          if (valid) {
            ...
            if (this.handleStatus === 'edit') {
              let params = {
                id: this.formItem.id,
                dataCode: this.formItem.dataCode,
    ...
    async handleSearch (page) {
      ...
      let res = await getDataDomain(params)
    ...
    handleRemove (data) {
      this.$Modal.confirm({
        ...
        onOk: async () => {
          ...
          let res = await deleteDataDomain(params)
          if (res.success) {
