<!--
 * @Author: 任继民
 * @Date: 2020-11-25 18:29:35
 * @LastEditors: 任继民
 * @LastEditTime: 2021-04-25 10:30:16
 * @Description:
-->
<template>
  <div>
    <HeaderContent>
      <Form ref="searchForm" :model="pageInfo" inline :label-width="80">
        <FormItem label="控制类型">
          <Select v-model="pageInfo.user_check_type" style="width: 150px">
    ...
    <Card shadow>
      <div class="search-con search-con-top">
        ...
        <div>
          <Button  icon="md-add" type="primary" @click="handleModal({},'add')">添加自定义规则</Button>
        </div>
      </div>
    ...
    <Modal v-model="modalVisible" :title="modalTitle" width="602" @on-cancel="handleReset" :mask-closable="false">
      <div>
        ...
        >
          ...
          <FormItem label="服务:" prop="service_check_type">
...
export default {
  ...
  data () {
    ...
    return {
      ...
      formItemRules: {
        ...
        user_check_type: [
          { type:'number', required: true, message: '请选择控制类型',  trigger: 'change' },
        ],
        user_id_or_ip: [
        ...
        service_check_type: [
          { type:'number', required: true, message: '请选择服务', trigger: 'change' },
        ],
        service_id: [
          { required: true, message: '请选择服务列表', trigger: 'change' },
        ],
        time_limit_start: [
          { required: true, message: '请选择限制开始时间', trigger: 'change', pattern: /.+/ },
          { required: true, message: '开始时间不能大于结束时间', trigger: 'change', validator: (rule, value, callback) => {
            let end = this.formItem.time_limit_end
        ...
        time_limit_end: [
          { required: true, message: '请选择限制结束时间', trigger: 'change',pattern: /.+/ },
          { required: true, message: '结束时间不能小于开始时间', trigger: 'change', validator: (rule, value, callback) => {
            let start = this.formItem.time_limit_start
        ...
        remark: [
          { pattern: /^[^\s]*$/, message: '专题名称不能输入空格', trigger: 'blur' },
        ]
      },
      formItem: {
        subjectName: ''
  ...
  methods: {
    ...
    handleStartTime(value) {
      this.pageInfo.create_date_begin = value;
      this.endOptions = {
        disabledDate:date =>{
          if (this.pageInfo.create_date_begin) {
            return date && date.valueOf() < new Date(this.pageInfo.create_date_begin).getTime()
          }else {
            return date && date.valueOf() > Date.now();
          }
        }
    ...
    handleEndTime(value) {
      this.pageInfo.create_date_end = value;
      this.startOptions = {
        disabledDate:date =>{
          if (this.pageInfo.create_date_end) {
            return date && date.valueOf() > new Date(this.pageInfo.create_date_end).getTime() - 86400000
          }else {
            return date && date.valueOf() > Date.now();
          }
        }
