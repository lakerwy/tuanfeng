 ...
 * @LastEditors: 任继民
 * @LastEditTime: 2020-11-17 10:49:21
 * @Description:
-->
<template>
  <div>
    <HeaderContent>
      <Form ref="searchForm" :model="pageInfo" inline :label-width="80">
        ...
        <FormItem label="上级类型" prop="start">
          <Select>
    ...
    <Card shadow>
      ...
      <Table border :columns="columns" :data="data" :loading="loading">
        ...
        <template slot="create_date" slot-scope="{ row }">
          <span>{{row.create_date? row.create_date.replace("T", ' '):''}}</span>
        </template>
        <template slot="action" slot-scope="{ row }">
          <img src="../../../assets/images/bianji.png" alt="" srcset="" @click="handleModal(row, 'edit')">
          <a @click="handleTabClick(row, 'stop')" >停止</a>
          <img src="../../../assets/images/shanchu.png" alt="" srcset="" @click="handleModal(row, 'del')">
        </template>
      </Table>
      <Page
        transfer
    ...
    <Modal v-model="modalVisible" :title="modalTitle" width="60%" @on-cancel="handleReset" :mask-closable="false">
      <div>
        <Form
        ...
        >
          ...
          <FormItem label="上级类型:" prop="code">
            <Select>
              <Option v-for="item in allServiceType" :key="item.code" :value="item.id">{{item.name}}</Option>
            </Select>
          </FormItem>
          <FormItem label="排序:" prop="show_index">
            <Input v-model.number="formItem.show_index" placeholder="请输入排序" />
          </FormItem>
          <FormItem label="备注:" prop="remark">
            <Input maxlength="200" show-word-limit :rows="4" v-model="formItem.remark" placeholder="请输入备注" />
          </FormItem>
        </Form>
  ...
  data () {
    ...
    return {
      ...
      formItemRules: {
        ...
        remark: [
          { pattern: /^[^\s]*$/, message: '编码不能输入空格', trigger: 'blur' },
        ]
      },
      formItem: {
        roleId: '',
