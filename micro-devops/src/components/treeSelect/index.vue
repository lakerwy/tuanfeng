<template>
  ...
  >
    <div
      tabindex="0"
      class="ivu-select-selection ivu-form-item-content"
    >
      <div
        @mouseover="mouseover"
        @mouseleave="mouseleave"
      >
      <span v-if="!multiple && queryVal!=''" @click="clickInputShow" class="ivu-select-selected-value">{{queryVal}}</span>
        <div @click="clickInputShow" v-else>
          <span
        ...
        <i
          :class="'ivu-icon ivu-icon-' +iconVal+ ' ivu-select-arrow'"
          @click="clickIcon"
        ></i>
      </div>
      <div
    ...
    selectChange(obj,arr) {
      ...
      if (this.multiple) {
        ...
        for (let i = 0; i < obj.length; i++) {
          ...
          this.multipleHideVal.push(obj[i].name)
        }
        ...
        this.$emit("on-select-change", this.queryVal, arr);
      }
      // this.$emit("on-select-change", hideVal, arr);
    },
    // 点击图标的时候展示树形菜单
    clickInputShow() {
      if (this.disabled) {
